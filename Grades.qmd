...
grades%>%
  filter(!is.na(Exam)) %>%
  ggplot(aes(y=Exam, fill=as.character(Group))) +
  geom_bar()+
  geom_text( stat = "count",  aes(label = stat(count)),position = position_stack(vjust = 0.5),color = "white", size = 3) +
  geom_hline(yintercept=10, color="green", size=1, linetype="dashed")+
     labs(title ="Grade by group",subtitle = "(the green line emphasizes the moment when a grade = 10)", x="Count", y="Grades") +
...
grades %>%
  ...
  geom_text(stat = "count", aes(label = as.character(Group)), position = position_fill(vjust = 0.5),color = "white",size = 3) +
  labs(title = "Grade by Group", x = "Proportion", y = "Grades") +
  theme(legend.position = "bottom") +
  theme_minimal()
