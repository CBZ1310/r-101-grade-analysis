...
The number of missing grades, when all value = NA is are taking into account, is `r {sum(is.na(grades_long_format$value))}`.

```{r}
sum(is.na(filter(grades_long_format, Studied_Variable == "Exam" )$value))
```


The number of missing grande, when only we take into account the value of Exam wich are equal to NA is `r {sum(is.na(filter(grades_long_format, Studied_Variable == "Exam" )$value))}`.
